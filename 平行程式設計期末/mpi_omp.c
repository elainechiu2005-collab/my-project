...
static void simulate_v4_mpi_omp(Particle* p, int N, double dt, double space_size,
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        if (rank == 0)
            printf("Usage: OMP_NUM_THREADS=<T> mpirun -np <P> %s <N> [steps]\n", argv[0]);
    ...
    for (int s = 0; s < steps; s++)
        simulate_v4_mpi_omp(particles, N, dt, space_size, rank, size, &comm_time);
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        printf("[MPI+OMP]  N=%-8d Steps=%-4d Procs=%-3d Threads=%-3d Compute=%.4f s  Comm=%.4f s  Total=%.4f s\n",
               N, steps, size, nthreads, elapsed - max_comm, max_comm, elapsed);
