...
static void simulate_v1_serial(Particle* p, int N, double dt, double space_size) {
...
int main(int argc, char *argv[]) {
    ...
    for (int s = 0; s < steps; s++)
        simulate_v1_serial(particles, N, dt, space_size);
    clock_gettime(CLOCK_MONOTONIC, &t1);

    double elapsed = (t1.tv_sec - t0.tv_sec) + (t1.tv_nsec - t0.tv_nsec) / 1e9;
    printf("[Serial]   N=%-8d Steps=%-4d Threads=1  Compute=%.4f s  Comm=0.0000 s  Total=%.4f s\n",
           N, steps, elapsed, elapsed);

    /* correctness checksum: sum of all particle positions after simulation */
