...
static void simulate_v2_omp(Particle* p, int N, double dt, double space_size) {
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        printf("Usage: OMP_NUM_THREADS=<T> %s <N> [steps]\n", argv[0]);
        return 1;
    }
    int N     = atoi(argv[1]);
    ...
    for (int s = 0; s < steps; s++)
        simulate_v2_omp(particles, N, dt, space_size);
    double elapsed = omp_get_wtime() - t0;

    printf("[OMP]      N=%-8d Steps=%-4d Threads=%-3d Compute=%.4f s  Comm=0.0000 s  Total=%.4f s\n",
           N, steps, nthreads, elapsed, elapsed);
